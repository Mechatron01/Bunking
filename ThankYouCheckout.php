...
//schedule order data
$isSchedule = isset($_REQUEST['schedule_order']) && $_REQUEST['schedule_order'] == "on" ? 1 : 0;
 $date = isset($_REQUEST['schedule_date']) ? $_REQUEST['schedule_date'] : '';
 $time = isset($_REQUEST['schedule_time']) ? $_REQUEST['schedule_time'] : '';

 if (empty($date)) {
    $date = date('Y-m-d'); // Set to current date
}

if (empty($time)) {
    $time = date('H:i:s'); // Set to current time
}
...
$Q_insert_userorder = 'INSERT INTO userorder (`userID`, total, address, phone, status, isSchedule, date, time) 
VALUES (' . $_SESSION['userID'] . ',' . $_SESSION['total_price'] . ',"' . $_POST['address_checkout'] . '","' . $_SESSION['phone'] . '", "successful", "' . $isSchedule . '","'.$date. '" , "' . $time . '") ' ;
...
$Q_select_orderID = 'SELECT max(orderID) FROM userorder WHERE userID =' . $_SESSION['userID'];
...
// $Q_insert_into_orders = 'INSERT INTO orders (userID, orderID, address, phone, isSchedule, date, time, productID, p_name, quantity, total)
// SELECT userorder.userID,userorder.orderID, userorder.address, userorder.phone, userorder.isSchedule, userorder.date, userorder.time ,orderitem.productID, products.p_name,orderitem.quantity,userorder.total
// FROM userorder
// JOIN orderitem ON userorder.orderID = orderitem.orderID
// JOIN products ON orderitem.productID = products.productID;';

// $run_insert_into_orders = mysqli_query($conn, $Q_insert_into_orders);
...
<head>
    ...
    <title>Bun King Bakery | Thank you!</title>
