...
$fname = $lname = $email = $address = $country = $city = $zip = $paymentMethod = $ccname = $ccnum = $ccexp_m = $ccexp_y = $cccvv = $isSchedule = $date = $time ="";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if ($fnameErr == "" && $lnameErr == "" && $emailErr == "" && $addressErr == "" && $countryErr == "" && $cityErr == "" && $zipErr == "" && $paymentMethodErr == "" && $ccnameErr == "" && $ccnumErr == "" && $ccexpErr == "" && $cccvvErr == "") {
    //$validated = true;
    header("location: ThankYouCheckout.php");
...
<head>
  ...
  <title>Bun King Bakery | Checkout</title>
...
<body>
  ...
  <div class="container">
    ...
    <main>
      ...
      <div class="py-5 text-center">
        <h1 class="business-name">Bun King Bakery</h1>
        <h2>Checkout form</h2>
      </div>
      ...
      <div class="row g-3">
          ...
          <ul class="list-group mb-3">
            ...
            ?>
                  ...
                  <li class="list-group-item d-flex justify-content-between lh-sm linen-rows">
                    ...
                    <span class="text-muted price-tag">CAD <?php echo number_format(($product['quantity']  * $product_row['p_price']), 2); ?></span>
                  </li>
            ...
            <li class="list-group-item d-flex justify-content-between total-row">
              <span>Total (CAD)</span>
              <strong class="price-tag">CAD <?php echo number_format($_SESSION['total_price'], 2); ?></strong>
            </li>
          </ul>
        ...
        <div class="col-md-7 col-lg-8">
          ...
          <form action="
            ...
            <div class="my-3">
              <div class="form-check">
                <input id="credit" name="paymentMethod" type="radio" class="form-check-input" value="creditCard" <?php if ($paymentMethod == "creditCard") {
                                                                                                                    echo "checked";
                                                                                                                  } ?>>
              ...
              <!-- MCB JUICE  -->
              <div class="form-check">
                <input id="mcbjuice" name="paymentMethod" type="radio" class="form-check-input" value="JuiceByMCB" <?php if ($paymentMethod == "JuiceByMCB") {
                                                                                                                      echo "checked";
                                                                                                                    } ?>>
              ...
              <!-- PAYPAL  -->
              <div class="form-check">
                <input id="paypal" name="paymentMethod" type="radio" class="form-check-input" value="paypal" <?php if ($paymentMethod == "paypal") {
                                                                                                                echo "checked";
                                                                                                              } ?>>
            ...
            <div class="form-check">
              <input type="checkbox" class="form-check-input" name="address_check" id="schedule-order" value=php echo $isSchedules; ?>"--
              <label class="form-check-label" for="schedule-order">
                <h5>Schedule this order</h5>
              </label>
            </div>
                                                                                                            -->
           ...
           <!-- <form method="post">-->
              <label for="schedule-order"><h5>Schedule this order</h5></label>
              <input type="checkbox" class="form-check-input" name="schedule_order" id="schedule-order">

              <div id="schedule-options" style="display:none;">

                <label for="schedule-date" class="form-label">Date:</label>
                <input type="date" name="schedule_date" id="schedule-date" class="form-control">
                <br>

                <label for="schedule-time" class="form-label">Time:</label>
                <input type="time" name="schedule_time" id="schedule-time" class="form-control">
            ...
            <button  class="w-100 btn btn-primary btn-lg button" type="submit">Continue to checkout
            </button>

            <a href="index.php" class="w-10 btn btn-primary btn-lg button mt-3 cancel">Cancel</a>
    ...
    <footer class="my-5 pt-5 text-muted text-center text-small">
      <p class="mb-1">&copy; 2023 Bun King Bakery</p>
  ...
  <!--Schedule Order Display Date and Time -->


  <script>
    window.addEventListener('DOMContentLoaded', () => {
      const checkbox = document.getElementById('schedule-order');
